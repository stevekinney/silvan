...
cli
  ...
  .action(async (name: string, options: CliOptions) => {
    ...
    const target = worktrees.find(
      (worktree) => worktree.branch === name || worktree.path.endsWith(name),
    );
...
cli.command('ui', 'Launch the Ink dashboard').action(async () => {
  const ctx = await createRunContext({ cwd: process.cwd(), mode: 'ui' });
  mountDashboard(ctx.events.bus, ctx.state);
  await ctx.state.lockRelease();
});
