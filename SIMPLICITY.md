...
- [x] Move tool definitions in `src/agent/registry.ts` to a single array and keep policy checks in one function.
- [x] Create a single `isProviderEnabled(config, provider)` helper and use it across `task/resolve` and lifecycle.
- [x] Group CLI commands into `src/cli/commands/*` and keep shared formatting in `src/cli/output/*`.
- [x] Create `buildQueueRequestView(request, config, nowMs)` and reuse in CLI + UI.
- [x] Replace large option bags in `executePlan` and run-controller with smaller explicit parameters.
- [x] Add a file-size guard (lint rule or CI script) to flag new files > 800 lines.
- [x] Standardize event payload builders for core phases to reduce ad-hoc event shapes.
- [x] Delete or inline any helper used only once after the refactor.
